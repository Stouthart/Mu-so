...
# Send HTTP request - <uri> [method] [output] [body]
http() {
  wget -q --no-config --no-netrc --no-hsts --no-cookies --no-proxy -4 -T2 -U '' -t1 \
    --method="${2:-GET}" -O "${3:-/dev/null}" ${4:+--body-data="$4"} "$BASE/$1" || error $?
}
